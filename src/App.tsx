...
function App() {
  ...
  const rankedInsights = useMemo(
    () => [...filteredInsights].sort((a, b) => b.property.overall_sentiment_score - a.property.overall_sentiment_score),
    [filteredInsights]
  );
            ...
            <div className="grid grid-cols-1 gap-8 xl:grid-cols-3">
              ...
              >
                <h3 className="mb-8 flex items-center gap-2 text-xl font-bold text-stone-900">
                  ...
                  Property Sentiment Leaderboard ({rankedInsights.length})
                </h3>
              ...
              <section className="animate-slide-up rounded-2xl border border-stone-200 bg-white p-8 shadow-sm xl:col-span-3" style={{ animationDelay: '0.6s' }}>
                <h3 className="mb-6 text-xl font-bold text-stone-900">Property Cards</h3>
                <p className="mb-6 text-sm text-stone-500">
                  All cards are generated directly from your property dataset, so any new property added to the data source appears here automatically.
                </p>
                <div className="grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-3">
                  {rankedInsights.map((insight) => (
                    <PropertyCard key={insight.property.id} property={insight.property} onClick={() => setSelectedProperty(insight.property)} />
                  ))}
                </div>
              </section>
            </div>
